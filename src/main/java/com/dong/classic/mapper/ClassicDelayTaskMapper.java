...
public interface ClassicDelayTaskMapper {
     ...
     */
    int updateStatus(@Param("taskNo") String taskNo, @Param("status") int status, @Param("actualTime") LocalDateTime actualTime);
     ...
     */
    int markConsumedByPayload(@Param("payload") String payload, @Param("actualTime") LocalDateTime actualTime);

    /**
     * 扫描已到预计时间却仍未消费的任务，用于补偿重投。
     */
    List<ClassicDelayTask> selectOverdue(@Param("threshold") LocalDateTime threshold, @Param("limit") int limit);
