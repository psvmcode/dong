...
public interface CrossBorderReconMapper {
     ...
     * 只取已结算的单子，未结算的不参与对账。
     */
    List<CrossBorderRemittance> selectSettledByBatch(@Param("batchNo") String batchNo, @Param("status") RemittanceStatus status);

    /**
     * 查询某账户某时间段的贷方流水总额，用于与渠道回单的入账金额比对。
     */
    BigDecimal sumCreditByAccountAndTime(@Param("accountId") Long accountId, @Param("from") LocalDateTime from, @Param("to") LocalDateTime to);

    /**
     * 按清算渠道统计已结算的笔数与总额，生成渠道维度的对账汇总。
     */
    List<java.util.Map<String, Object>> summaryByChannel(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to, @Param("status") RemittanceStatus status);
